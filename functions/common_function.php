  ...
  function get_all_products(){
    ...
    if (!isset($_GET['category'])) {
      if (!isset($_GET['brand'])) {
        ...
        $select_query = "Select * from `products` order by brand()";
        $result_query = mysqli_query($con, $select_query);
        $num_of_rows = mysqli_num_rows($result_query);
...
function cart(){
  if (isset($_GET['add_to_cart'])) {
    ...
    $select_query = "Select * from `cart_details` where ip_address = '$get_ip_adress'
    ...
    if ($num_of_rows>0) {
      ...
      values ($get_product_id, '$get_ip_adress', 0)";
...
function total_cart_price(){
  ...
  $get_ip_adress = getIPAddress();
  $total_price = 0;
  $cart_query = "Select * from `cart_details` where ip_address = '$get_ip_adress'";
      ...
      if (!isset($_GET['my_orders'])) {
        if (!isset($_GET['delete_account'])) {
          ...
          if ($row_count > 1) {
            ...
            echo "<h3 class = 'text-center text-success mt-5 mb-2'>You have <span class = 'text-danger'>zero</span> pending order</h3>
            <p class = 'text-center'><a href='../index.php?my_oders' class = 'text-dark'>Explore Products</a></p>";           
          }
        }
      }
    }
